...
Clients and bzbd speak newline-delimited JSON over `bzbd.sock`, one UTF-8 message per line. The client's first line is `{"hello": <protocol_version>}`; bzbd answers `Pong { version, pid }`, or `Error` when it does not speak that version, and closes. A line may not exceed 64 KiB; bzbd answers `Error` and closes rather than buffering a message that never ends.
